...
object Common {
  ...
  def connMarketTable(table: String): ConnTable  = ConnTable(connMarket, table)
  def connProductTable(table: String): ConnTable = ConnTable(connProduct, table)
  def connBizTable(table: String): ConnTable     = ConnTable(connBiz, table)
}

...
object AShareInformationCitics extends Information {
  ...
  lazy val readFromCol = connMarketTableColumn("ASHAREDESCRIPTION", Token.timeColumnMarket)
  lazy val saveToCol   = connBizTableColumn("ashare_information_citics", Token.timeColumnBiz)
  lazy val primaryKey  = ("PK_ashare_information_citics", Seq(Token.objectId))
  lazy val index1      = ("IDX_ashare_information_citics_1", Seq(Token.timeColumnBiz))
  lazy val index2      = ("IDX_ashare_information_citics_2", Seq(Token.symbol))

  lazy val compoundPK = Seq("object_id_ad", "object_id_aim", "object_id_ic")
  lazy val conversionFn = RegimeFn
    .dropNullRow(compoundPK)
    .andThen(RegimeFn.concatMultipleColumns(Token.objectId, compoundPK, concatenateString))
